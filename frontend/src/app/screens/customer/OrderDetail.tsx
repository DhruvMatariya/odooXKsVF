...
import { ORDERS, RETURN_SLOTS } from '../../lib/mockData';
import { StatusBadge } from '../../components/shared/StatusBadge';
import { OrderTimeline } from '../../components/shared/OrderTimeline';
import { formatDate, formatDateTime, formatPrice, formatPricingLabel } from '../../lib/utils';
    ...
    <div style={{ maxWidth: '960px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '24px', flexWrap: 'wrap', gap: '12px' }}>
        ...
        <div style={{ display: 'flex', gap: '8px' }}>
          {status === 'DISPATCHED' && (
            <ActionBtn label="Confirm Delivery" onClick={() => setModal('confirm-delivery')} />
          )}
          {(status === 'ACTIVE_RENTAL' || status === 'HANDED_OVER') && (
            <>
              {status === 'ACTIVE_RENTAL' && <ActionBtn label="Schedule Return" onClick={() => setModal('schedule-return')} secondary />}
              <ActionBtn label="Report Issue" onClick={() => setModal('report-issue')} warning />
            </>
          )}
        </div>
      </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          <Section title="Product">
            <div style={{ display: 'flex', gap: '16px', alignItems: 'center' }}>
              <img src={order.product.thumbnail} alt={order.product.name} style={{ width: '80px', height: '60px', objectFit: 'cover', borderRadius: '6px', flexShrink: 0 }} />
              <div>
                ...
                <div style={{ fontSize: '13px', color: '#738A6E', marginTop: '2px' }}>by {order.product.vendor.companyName}</div>
              </div>
            </div>
          </Section>
          ...
          {order.payment && (
            <Section title="Payment">
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                <DetailRow label="Amount" value={formatPrice(order.payment.amount)} />
            ...
            <div style={{ padding: '20px 24px' }}>
              ...
              {modal === 'schedule-return' && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                  ...
                  {RETURN_SLOTS.slice(0, 6).map(slot => {
                    const pct = slot.bookedCount / slot.capacity;
                    const isFull = pct >= 1;
                    return (
                      <button key={slot.id} onClick={() => !isFull && setSelectedSlot(slot.id)} disabled={isFull} style={{
                        padding: '12px', borderRadius: '8px',
                        border: `1px solid ${selectedSlot === slot.id ? '#738A6E' : '#E4E7E2'}`,
                        background: selectedSlot === slot.id ? 'rgba(115,138,110,0.06)' : '#FAFAF8',
                        cursor: isFull ? 'not-allowed' : 'pointer', textAlign: 'left',
                      }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px' }}>
                          <span style={{ fontSize: '13px', fontWeight: 600, color: isFull ? '#BFCFBB' : '#344C3D' }}>
                            {slot.date} — {slot.slotLabel}
                          </span>
                          <span style={{ fontSize: '11px', color: isFull ? '#C97B3D' : '#8EA58C' }}>
                            {slot.bookedCount}/{slot.capacity} booked
                          </span>
                        </div>
                        <div style={{ height: '4px', background: '#E4E7E2', borderRadius: '2px', overflow: 'hidden' }}>
                          <div style={{ height: '100%', width: `${Math.min(pct * 100, 100)}%`, background: pct >= 0.8 ? '#C97B3D' : '#738A6E', borderRadius: '2px', transition: 'width 0.3s' }} />
                        </div>
                      </button>
                    );
                  })}
...
function ActionBtn({ label, onClick, secondary, warning }: { label: string; onClick: () => void; secondary?: boolean; warning?: boolean }) {
  return (
    <button onClick={onClick} style={{
      ...
      borderColor: warning ? '#C97B3D' : secondary ? '#E4E7E2' : 'transparent',
      background: warning ? 'rgba(201,123,61,0.08)' : secondary ? '#fff' : '#738A6E',
      color: warning ? '#C97B3D' : secondary ? '#344C3D' : '#fff',
